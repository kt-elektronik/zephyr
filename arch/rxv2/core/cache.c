/* cache.c - d-cache support for Renesas RX65N CPUs */
...
// check if: TODO: verify constants
...
static bool dcache_available(void){
	// unsigned long val = z_arc_v2_aux_reg_read(_ARC_V2_D_CACHE_BUILD); // check if: TODO: an rxv2 equivalent for this function is required here

	unsigned long val = 0; // check if: TODO: an rxv2 equivalent for this function is required here
...
static void dcache_dc_ctrl(uint32_t dcache_en_mask){
	if (dcache_available()) {
		// z_arc_v2_aux_reg_write(_ARC_V2_DC_CTRL, dcache_en_mask); // check if: TODO: an rxv2 equivalent for this function is required here
	}
}
...
void arch_dcache_flush(void *start_addr_ptr, size_t size){
	...
	do {
		// z_arc_v2_aux_reg_write(_ARC_V2_DC_FLDL, start_addr); // check if: TODO: an rxv2 equivalent for this function is required here
...
//		do {
//			if ((z_arc_v2_aux_reg_read(_ARC_V2_DC_CTRL) & // check if: TODO: an rxv2 equivalent for this function is required here
...
void arch_dcache_invd(void *start_addr_ptr, size_t size){
	...
	do {
//		z_arc_v2_aux_reg_write(_ARC_V2_DC_IVDL, start_addr); // check if: TODO: an rxv2 equivalent for this function is required here
//		__builtin_arc_nop();
//		__builtin_arc_nop();
//		__builtin_arc_nop();
		start_addr += DCACHE_LINE_SIZE;
...
static void init_dcache_line_size(void){
	...
	val = z_arc_v2_aux_reg_read(_ARC_V2_D_CACHE_BUILD); // check if: TODO: an rxv2 equivalent for this function is required here
