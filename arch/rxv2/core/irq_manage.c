...
void arch_irq_enable(unsigned int irq)
{
	__ASSERT(irq < CONFIG_NUM_IRQS,
		"trying to enable invalid interrupt (%u)", irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR,
		"trying to enable reserved interrupt (%u)", irq);

	uint32_t key = irq_lock();

	/* reset interrupt before activating */
	WRITE_BIT(REG(IR_BASE_ADDRESS + irq), 0, false);
	WRITE_BIT(REG(IER_BASE_ADDRESS + REG_FROM_IRQ(irq)), BIT_FROM_IRQ(irq),
		true);
	irq_unlock(key);
}
...
{
	__ASSERT(irq < CONFIG_NUM_IRQS,
		"trying to disable invalid interrupt (%u)", irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR,
		"trying to disable reserved interrupt (%u)", irq);
...
{
	__ASSERT(irq < CONFIG_NUM_IRQS,
		"is_enabeld on invalid interrupt (%u)",	irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR,
		"is_enabeld on reserved interrupt (%u)", irq);

	return (REG(IER_BASE_ADDRESS + REG_FROM_IRQ(irq)) &
		BIT(BIT_FROM_IRQ(irq))) != 0;
}
...
{
	__ASSERT(irq < CONFIG_NUM_IRQS,
		"irq_priority_set on invalid interrupt (%u)", irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR,
		"irq_priority_set on reserved interrupt (%u)", irq);
	__ASSERT(prio < CONFIG_NUM_IRQ_PRIO_LEVELS,
		 "invalid priority (%u) for interrupt %u", prio, irq);
...
{
	__ASSERT(irq < CONFIG_NUM_IRQS,
		"trying to connect to invalid interrupt (%u)", irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR,
		"trying to connect to reserved interrupt (%u)", irq);
