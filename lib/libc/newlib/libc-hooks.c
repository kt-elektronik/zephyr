...
void __retarget_lock_init(_LOCK_T *lock)
{
	...
	*lock = (_LOCK_T)malloc(sizeof(struct k_sem));
#else
	*lock = (_LOCK_T)k_object_alloc(K_OBJ_SEM);
#endif /* !CONFIG_USERSPACE */
...
#ifndef CONFIG_USERSPACE
	*lock = (_LOCK_T)malloc(sizeof(struct k_mutex));
#else
	*lock = (_LOCK_T)k_object_alloc(K_OBJ_MUTEX);
#endif /* !CONFIG_USERSPACE */
